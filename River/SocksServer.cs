...
namespace River
{
	public class SocksServer : IDisposable
	{
		...
		class ClientWorker
		{
			...
			private byte[] _bufferForwardRead = new byte[1024];
			private int _bufferReceivedCount;
			private int _bufferProcessedCount;
			private SocksSessionState _sessionState;
			...
			public ClientWorker(TcpClient client)
			{
				...
				_stream.BeginRead(_buffer, _bufferReceivedCount, _buffer.Length, Received, null);
			}
			...
			private void Received(IAsyncResult ar)
			{
				...
				switch (_sessionState)
				{
					case SocksSessionState.JustConnected:
									...
									if (EnsureReaded(8))
									{
										...
										try
										{
											...
											if (_bufferProcessedCount < _bufferReceivedCount)
											{
												...
												_clientForward.GetStream()
													.Write(_buffer, _bufferProcessedCount, _bufferReceivedCount - _bufferProcessedCount);
											}
											_clientForward.GetStream()
												.BeginRead(_bufferForwardRead, 0, _bufferForwardRead.Length, ReceivedFromForwarder, null);
											_sessionState = SocksSessionState.Streaming;
						...
						break;
					case SocksSessionState.Streaming:
						_clientForward.GetStream().Write(_buffer, 0, count);
						_stream.BeginRead(_buffer, 0, _buffer.Length, Received, null);
						break;
					default:
			...
			{
				var count = _clientForward.GetStream().EndRead(ar);
			...
			private bool EnsureReaded(int readed)
			{
				...
				{
					_stream.BeginRead(_buffer, _bufferReceivedCount, _buffer.Length - _bufferReceivedCount, Received, null);
					return false;
				}
				return true;
			}
